---
title: "Creating Hotgenes Objects"
output:
  github_document:
    html_preview: false
vignette: >
  %\VignetteIndexEntry{Creating Hotgenes Objects}
...
knitr::opts_chunk$set(
  collapse = TRUE,
  fig.path = "01_Creating_Hotgenes_Objects_files/figure-",
...
```{r dromics_pipeline, message=FALSE, warning=FALSE}
library(Hotgenes)
library(DRomics)
...
dose-responsive feature. The `log2FoldChange` is computed as the
log2 ratio between the expression at the maximum (or extreme) dose and
the baseline, using a small pseudocount for count data.
...
```{r deseq2_pipeline, message=FALSE, warning=FALSE}
library(DESeq2)
...
```{r deseq2_hotgenes, message=FALSE, warning=FALSE}
...
```{r limma_load, message=FALSE, warning=FALSE}
...
```{r limma_prep, message=FALSE, warning=FALSE}
...
```{r limma_model, message=FALSE, warning=FALSE}
# VSN normalisation
