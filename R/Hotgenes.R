...
#' estimator. Other options are "apeglm" and "ashr".
#' @param contrasts values from DESeq2 resultsNames function. If NULL
#' (default) DE for all results will be reported.
#' @param contrast_vectors Named list of contrast vectors, each of the form
#' \code{c("factor", "level1", "level2")}, passed individually to
#' \code{\link[DESeq2]{lfcShrink}} as the \code{contrast} argument.
#' Requires \code{lfcShrink_type = "normal"} (the default) or 
#'  \code{lfcShrink_type = "ashr"}; \code{"apeglm"}
#' does not support contrast vectors and will error.
#' Default is NULL.
...
HotgenesDEseq2 <- function(DEseq2_object = NULL,
  ...
  # contrast_vectors requires:
  contrast_v_req <- c("normal", "ashr")
  
  if (!is.null(contrast_vectors) && !lfcShrink_type %in% contrast_v_req) {
    cli::cli_abort(c("contrast_vectors always use lfcShrink types: {contrast_v_req}." ,
    x =  "You've set lfcShrink_type = '{lfcShrink_type}'."))
  ...
  # setting method for contrast vector
  if (methods::is(contrast_vectors, "list") && !is.null(names(contrast_vectors))) {
    output_method <- contrast_vectors %>%
